...
public class AStar : MonoBehaviour
{
    ...
    public List<Vector3> FindPath(Vector3 startPosition, Vector3 targetPosition)
    {
        ...
        GridNode startNode = GetGridNodeAtWorldPos(startPosition);
        GridNode targetNode = GetGridNodeAtWorldPos(targetPosition);
    ...
    public GridNode GetGridNodeAtWorldPos(Vector3 obj)
    {

        int x = Mathf.FloorToInt((obj.x / cellSize + (rows * 0.5f)) - (this.transform.position.x / cellSize));
        int y = Mathf.FloorToInt((obj.z / cellSize + (columns * 0.5f)) - (this.transform.position.y / cellSize));
    ...
    public void CheckFieldForWalkable()
    {
        ...
        {
            //node.gridPosition
            if (Physics.CheckBox(new Vector3(GetWorldPosFromGridPos(node).x, transform.position.y, GetWorldPosFromGridPos(node).y), new Vector3(cellSize*0.25f, cellSize*0.25f, cellSize*0.25f), transform.rotation, unwalkableLayerMask))
            {
